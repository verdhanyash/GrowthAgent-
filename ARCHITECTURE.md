...
|---|-----------|------|---------------|
| 1 | `catalog-intelligence-agent` | NIM · llama-3.3-70b | Offline enrichment of messy raw product rows → descriptions, categories, tags, occasions, pairings. **Structurally powerless**: never touches price/cost/margin/stock (§7.8). |
...
| 3 | `negotiation-upsell-agent` | NIM · llama-3.3-70b | The reasoning centerpiece. Evidence Pack in → cited PROPOSED cart out. Never an approved cart. |
| 4 | `campaign-orchestrator-agent` | NIM · llama-3.3-70b (rationales only) | Mines synthetic sales autonomously → PrioritySet injected into negotiation context; every decision carries a plain-language rationale with provenance. |
| 5 | `explainer-agent` | NIM · llama-3.3-70b | Turns any decision (any agent's output, any gatekeeper verdict) into human-readable audit narrative — always typed `non_authoritative: true`. |
...
flowchart TD
    ...
    D --> E[negotiation-upsell-agent<br/>NIM · llama-3.3-70b · guided_json<br/>per-claim evidence citations]
...
**Invocation:** one plain-fetch call to NIM's OpenAI-compatible `/chat/completions` (`meta/llama-3.3-70b-instruct`, sampling knobs omitted), structured output via `nvext.guided_json` over `toJSONSchema(NegotiationProposalZ)` with defensive re-validation (`parsed_output === null` routes to repair/fallback exactly as before), retries OFF in the seam (stage owns a 12s wall budget: ≤10.5s call → sync auditor → fallback build <10ms; max 1 retry on 429/408/409/5xx/network only, retry-after honored and capped at 1.5s; exactly one schema-repair attempt if budget allows). Prompt caching: NIM/OpenAI-compat exposes no explicit cache-control surface — the frozen-prefix discipline (byte-stable system prompt + canonical payload) is kept because it is what makes replay keys stable, but cache-hit telemetry (`usage.cache_read_input_tokens`) has no equivalent and reads as absent.
...
- Every LLM call goes through a transport seam: a live NIM adapter (`LiveNim*Port`/`LiveNimTransport`, plain fetch against the OpenAI-compatible endpoint — zero SDK dependency) or its replay twin (fixtures keyed by `sha256(canonicalJson(requestBody))`; negotiation keys on `system_prompt_hash ‖ pack_hash ‖ buyer_request_canonical`). Replay NEVER silently falls back to live — a surprise network call mid-demo is worse than a loud error. `npm run demo:record` records scenarios once on a good connection.
...
**Real:** Razorpay test-mode Orders API + HMAC webhook verification path · NVIDIA NIM live calls (llama-3.3-70b ×4 agents) with grammar-constrained structured outputs (`nvext.guided_json`) and defensive re-validation · deterministic gatekeeper + citation auditor + tagger · hash-chained audit · three-layer idempotency · atomic stock reservation · SSE protocol with resume.
...
| Metric value type | campaign.md §6.2 sketches `value: number`, but TIMING templates quote context metrics (`dow_label` → "Sundays", `occ_label` → "eggless") that have no numeric form | **`Metric.value: number \| string`** — numeric metrics keep full-precision numbers; string values admitted ONLY for label-like context keys; `display` stays the single auditable token (M3) |
| Opportunity metric sets | §8.1 vs §8.2 disagree on which metrics each type carries; an extra metric would make honest output AND its template unverifiable (verifier Rule 1 demands every display) | **EXACT-SET RULE**: each type carries EXACTLY the keys its template quotes — UNDERSELLING gains `stock_units` (§8.1's SKU-digit-legitimization beats §8.2's older sketch; template extended to quote "60 units on hand"); analytics extending a set must extend template + table together (M3) |
| SKU identifiers in rationale verification | Verifier tokenizes numbers; SKU codes like `KAJU_KATLI_250G` tokenize into phantom digits ("250") that honest rationales naming the SKU would trip | **Strip SKU-like identifiers BEFORE lowercasing** (`\b[A-Z][A-Z0-9]*(?:[_-][A-Z0-9]+)+\b`, mirroring the negotiation auditor's scanner): names are not quantities; unseparated codes like EMPLOYEE50 keep their digits so injections still trip (M3) |
| Rationale retry ladder ownership | §7.3 says "retry with backoff" but not who retries; SDK has its own retry stack | **SDK client runs `maxRetries: 0`; OUR ladder owns attempts=2** with backoff 500ms·2^n+jitter (sleep/jitter injectable for determinism); PARSE_FAILED gets exactly ONE re-request; NON_RETRYABLE aborts immediately; CHAOS_FORCED breaks without retrying (M3) |
| Seed-time agent failure | §10 keeps previous set on LLM failure, but seed time HAS no previous set | **TEMPLATE_ONLY publish** when no previous set exists: all-template set ships so demo beat 1 always has material; `llm_invocation: null` marks it (ratifies §17 item 4) (M3) |
| set_id digest formulation | TS-side id derivation vs §9's SQL `left(md5(payload::text), 8)` could drift into two different ids for one set | **`set_id = ps_v<n>_<md5(canonicalJson(set-without-set_id))[:8]>`** on both sides — canonicalJson IS what we persist, so the formulations coincide by construction (M3) |
| TIMING action mapping | Four opportunity types but a three-member action enum | **TIMING reuses PRIORITIZE_IN_BUNDLES** (fixed ACTION_MAP); the timing reason rides in metrics + rationale, never in the action enum (M3) |
| §15-A fixture SKUs under combined assembly | Doc's dataset puts EX-1 expiry on E=KAJU_KATLI_250G while AT-1's pair is D+E — co-assembled they'd fight over E and suppress one opportunity | **Per-query fixtures only in the doc; unit fixtures de-conflicted** (expiry example on F=DRY_CAKE_ASSORTED) so the golden four-opportunity set assembles intact; shared-SKU conflict semantics stay covered by synthetic U-11 tests (M3) |
| UNENRICHED vs FAILED statuses | §2.5's enum has both, but no spec says which failures take which; only FAILED has a CHECK constraint | **ALL operational agent failures → `UNENRICHED` + error_detail** (parse, transport, validation — raw fields stay authoritative, matching the seed doc's degraded path and the frontend's degraded chip); **FAILED reserved for unexpected internal errors** (defensive catch-all), which also satisfies ce_failed_has_reason (M4) |
| Enrichment output enforcement point | Closed occasion vocabulary could be enforced IN the LLM schema (JSON-schema enum) or after decode | **Deterministic post-hoc filter with warnings** (`UNKNOWN_OCCASION`, `PAIRING_NOT_IN_CATALOG`, `TAGS_TRUNCATED`): partial accept beats re-request churn for marketing copy; the LLM-facing schema stays permissive, the DB row stays clean (M4) |
| Money-flavored prose from enrichment | Trust rule makes enrichment powerless structurally, but a model could still WRITE "₹299 only" into description | **Flag, don't strip**: MONEY_TOKEN_IN_COPY warning keeps the copy (no numeric column exists to feed) and makes the attempt visible in the audit trail; silent mutation of content is worse than a loud warning (M4) |
| Trust rule enforcement depth | negotiation.md §1.7 enforces via payload types + builder lint; prompt layer unspecified | **Trust rule AT THE SOURCE too**: `CatalogItemInput` has no commercial fields (sku/name/description/uom/category only), so no code path can transmit cost/list/stock/expiry to the model; pairing allow-listing matches merchant SKUs case-insensitively with canonical casing (M4) |
| SQL CHECK constraints in TS | catalog_enriched's ce_model_required / ce_failed_has_reason live in PG; TS writers could violate them until insert time | **Mirrored as superRefine on EnrichedSkuSchema** — a write PG would reject fails at the seam instead of at the database (M4) |
| Degraded narratives | api-contract + FE note 16 + gatekeeper.md leave "degraded explanation" wording ambiguous vs the raw-trace rule | **`degraded=true` NEVER ships in v1**: LLM success + verification pass → ONE narrative (`non_authoritative: true`, `degraded: false`); ANY failure or verifier rejection → NOTHING ships (raw rule-trace JSON stands). No templated fallback prose exists to mask failures (M5) |
| Untrusted-restatement scan granularity | FE §4.4.6 bans restating buyer text but not whether partial quotes count | **Whole-string fingerprint scan** (whitespace-collapsed, case-folded): the FULL untrusted string outside a `buyer claim —` span rejects; partial-fragment/paraphrase policing is deliberately out of scope — registered as the scan's honest boundary (M5) |
| Buyer-claim prefix window | An exact-prefix-length lookbehind chops the prefix's first char when the quote starts right after `prefix "` — laundering hole | **Generous 40-char lookbehind window** before each occurrence; deterministic and immune to off-by-one spacing (M5) |
| Event payload key-order in narration prompts | Producer key-order would perturb replay keys byte-for-byte | **Payloads embedded via `canonicalJson(e.payload)`** inside the user payload — key-order can never perturb the sha256 request key (M5) |
| Rejected narration text | A rejected narrative vanishes into "no explanation" with no trace of what the model tried | **Rejected text preserved verbatim** (first 200 chars) in the NONE reason — audit trail shows the attempt, containment pattern matches CAMPAIGN_RATIONALE_FALLBACK (M5) |
| LLM provider | Buildathon constraint shift: Anthropic SDK surface replaced mid-build | **NVIDIA NIM** (`meta/llama-3.3-70b-instruct`, all four agents): zero-dependency `fetch` client (`api/src/llm/nim.ts`) against NIM's OpenAI-compatible `/chat/completions`; env `NVIDIA_API_KEY` (+ optional `NIM_BASE_URL` for self-hosted containers); prompts/schemas/ladders/DEMO_STABLE_MODE untouched above the seam (M5b) |
| Structured-output enforcement point | Anthropic `zodOutputFormat` has no NIM equivalent | **Grammar-constrained decoding + defensive re-validation**: zod/v4 `toJSONSchema(schema)` rides `nvext.guided_json`; replies are fence-stripped/JSON-parsed and re-validated through the same strict schema — parse or schema failure throws the module's PARSE_FAILED class exactly as before. Guided decoding constrains, never guarantees, so validation is never skipped (M5b) |
| Provider failure taxonomy | Anthropic typed error classes (RateLimit/Internal/Connection…) don't exist on a raw-fetch client | **Status+class taxonomy on `NimHttpError`/`NimNetworkError`**: {408, 409, 429, ≥5xx, network/timeout} → retryable; other 4xx and UNKNOWN shapes → NON_RETRYABLE fail-fast; all four modules delegate to one shared `classifyNimTransport` — the identical-4-way-classification guarantee survives the swap (M5b) |
| Stop-reason vocabulary | stage.ts branches on `"max_tokens"`/`"refusal"`; NIM emits `length`/`content_filter` | **Translated IN the transport** (`mapStopReason`) so downstream business logic stays provider-neutral (M5b) |
| Prompt-cache telemetry | Anthropic cache_control breakpoints + `usage.cache_read_input_tokens` have no NIM/OpenAI-compat equivalent | **Honest flag**: frozen-prefix discipline kept (it stabilizes replay keys), but cache-hit telemetry reads as absent; §13 updated to say so rather than fake numbers (M5b) |
| Adaptive-thinking summaries | negotiation.md pinned adaptive/summarized reasoning for the UI pane; llama-3.3 emits no reasoning blocks | **Field shape preserved, content empty** (`thinking_summary: ""`) so TransportResult consumers are stable; the live-reasoning demo beat degrades to token-stream text only — registered as an honest capability delta (M5b) |
| Injection-note breadth on the wire | frontend-events.md §InjectionFlagged shipped `customer_note_full` and told the red banner to quote it VERBATIM — an unbounded attacker-controlled string fanned out to every SSE subscriber | **Bounded `customer_note_preview` (≤280) + `customer_note_len`**, with `matched_snippets` (≤8 × ≤160 chars) as the evidence surface; the full note stays server-side in `proposal_txs.request_bytes` so forensic value is retained without giving a hostile note a broadcast channel. Preview is CONTEXT, not evidence — a note can pad benign text past 280 chars (M7/security-audit P8) |
| Settlement buyer-route ownership | settlement.md's `GET /v1/tx/:tx_id` predates agent identity; api-contract.md §4 mandates per-agent scoping on buyer reads | **`transactions.agent_id` (V11, nullable) stamped by `settle()` from the authenticated caller**, and the read route answers a UNIFORM 404 for both "no such tx" and "not your tx" (never a distinguishable 403) — matching the M8 proposals/stream surfaces, which derive the same ownership from `proposal_idempotency`. NULL owner ⇒ owned by nobody ⇒ 404 for every caller (security-audit P2-full) |
